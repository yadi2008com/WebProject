...
window.onload=function ()
{
	...
	oBtnPrev.znsOpacity=0;
	oBtnNext.znsOpacity=0;
	...
	for(i=0;i<aLiBtn.length;i++)
	{
		...
		{
			if(g_iNowImg!=this.znsIndex)
			{
				showLiBtn(this.znsIndex);
			}
		};
		aLiBtn[i].onmouseout=function ()
		{
			if(g_iNowImg!=this.znsIndex)
			{
				hideLiBtn(this.znsIndex);
			}
		};
		aLiBtn[i].onmousedown=function ()
		{
			gotoImg(this.znsIndex);
		};
		g_aTimerBtn[i]=null;
		g_aLiBtnAlpha[i]=60;
	}
...
function showBtn(oBtn)
{
	if(oBtn.znsTimer)
	{
		clearInterval(oBtn.znsTimer);
	}
	
	oBtn.znsTimer=setInterval
	(
		function ()
		{
			if(oBtn.znsOpacity<100)
			{

				oBtn.znsOpacity+=10;
				oBtn.style.display='block';
				oBtn.style.filter="alpha(opacity:"+oBtn.znsOpacity+")";
				oBtn.style.opacity=oBtn.znsOpacity/100;
			}
			else
			{
				oBtn.style.filter="";
